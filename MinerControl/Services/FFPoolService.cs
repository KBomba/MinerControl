...
{
    public class FFPoolService : ServiceBase<FFPoolPriceEntry>
    {
        // http://ffpool.net/api/status
        // {
        ...
        public override void Initialize(IDictionary<string, object> data)
        {
            ...
            foreach (object rawitem in items)
            {
                ...
                FFPoolPriceEntry entry = CreateEntry(item);
        ...
        {
            ClearStalePrices();
            WebUtil.DownloadJson("http://ffpool.net/api/status", ProcessPrices);
            WebUtil.DownloadJson(string.Format("http://ffpool.net/api/wallet?address={0}", _account), ProcessBalances);
        }
        ...
        private void ProcessPrices(object jsonData)
        {
            ...
            lock (MiningEngine)
            {
                foreach (string key in data.Keys)
                {
                    ...
                    FFPoolPriceEntry entry = GetEntry(algo);
                    ...
                    entry.FeePercent = _account.Trim()[0] == '1' ? feePercent + 2.0M : feePercent;
                    // If conversion to BTC is needed (as most do), fee is +2%
                }
        ...
        private void ProcessBalances(object jsonData)
        {
            ...
            lock (MiningEngine)
            {
                ...
                foreach (FFPoolPriceEntry entry in PriceEntries)
                    entry.AcceptSpeed = 0;
                ...
                {
                    FFPoolPriceEntry entry = GetEntry(key.ToLower());
                    if (entry == null) continue;
